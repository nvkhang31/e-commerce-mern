import { resend } from "./config.js";
import {
  verificationTokenEmailTemplate,
  WELCOME_EMAIL_TEMPLATE,
} from "./email-templates.js";

export const sendVerificationEmail = async (email, verificationToken) => {
  try {
    const { data, error } = await resend.emails.send({
      from: "Acme <onboarding@resend.dev>",
      to: ["khangnvde170679@fpt.edu.vn"],
      subject: "Verify Your Email Address Now",
      html: verificationTokenEmailTemplate.replace(
        "{verificationToken}",
        verificationToken
      ),
    });

    if (error) {
      console.log("Resend API error:", error);
      throw new Error("Error sending verification email");
    } else {
      console.log("Verification email sent:", data);
    }
  } catch (error) {
    console.log("error sending verification email", error);
    throw new Error("Error sending verification email");
  }
};

export const sendWelcomeEmail = async (email, name) => {
  try {
    const { data, error } = await resend.emails.send({
      from: "Acme <onboarding@resend.dev>",
      to: ["khangnvde170679@fpt.edu.vn"],
      subject: "Welcome to our company",
      html: WELCOME_EMAIL_TEMPLATE.replace("{name}", name),
    });

    if (error) {
      console.log("Resend API error (welcome email):", error);
    } else {
      console.log("Welcome email sent:", data);
    }
  } catch (error) {
    console.log("error sending welcome email (catch):", error);
  }
};

export const sendPasswordResetEmail = async (email, resetURL) => {
  try {
    const { data, error } = await resend.emails.send({
      from: "Acme <onboarding@resend.dev>",
      to: ["khangnvde170679@fpt.edu.vn"],
      subject: "Reset Your Password",
      html: `Click <a href="${resetURL}">here</a> to reset your password`,
    });
    console.log("Password reset email - data:", data);
    console.log("Password reset email - error:", error);
  } catch (error) {
    console.log("error sending password reset email", error);
  }
};

export const sendResetSuccessEmail = async (email) => {
  try {
    const { data, error } = await resend.emails.send({
      from: "Acme <onboarding@resend.dev>",
      to: ["khangnvde170679@fpt.edu.vn"],
      subject: "Password Reset Was Successful",
      html: `Your password was reset successfully`,
    });
    console.log("Reset success email - data:", data);
    console.log("Reset success email - error:", error);
  } catch (error) {
    console.log("error sending password reset successful email", error);
  }
}